...
def push_data(args):
    ...
    for i in range(5):
        ...
        print
        "i=%s, status=%s, result=%s" % (i, status, result)
...
def main():
    ...
    if os_code == OS_UNKNOWN:
        print
        "脚本不支持该操作系统, 只支持centos, ubuntu, debian, redhat"
        sys.exit(1)
    ...
    if device_type != TYPE_VM:
        ...
        if re.search('(?i)^dell', Model):
            if not check_megacli(os_code):
                print
                "[error]: <DELL RAID>megacli install failed!"
                sys.exit(1)

            if not check_lsiutil():
                print
                "[error]: <DELL RAID>lsiutil install failed!"
                sys.exit(1)
